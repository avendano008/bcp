			...
			<!--end::Title-->
				...
				<script type="text/javascript">
					function date_range(department,month,day){
						location.href='?page=report_payroll&department='+department+'&month='+month+'&day='+day;
					}
				</script>
					...
					<tbody class="fw-bold text-gray-600">
						...
						if(isset($_GET['department'])){
							...
							$to=date_format($date_to,"m/d/Y");
								...
								$sql = "SELECT *,IFNULL((SELECT SUM(e_total_rate) FROM attendance_log a WHERE a.log_date BETWEEN '$from' AND '$to' AND a.e_id=e.id AND a.status='Approved' AND a.payroll_status!='1'),'00') AS salary  FROM employee e WHERE e.id='$department' ORDER BY id DESC";
								$result = $conn->query($sql);
						}else{

								$sql = "SELECT *,IFNULL((SELECT SUM(e_total_rate) FROM attendance_log a WHERE a.log_date BETWEEN '$from' AND '$to' AND a.e_id=e.id AND a.status='Approved' AND a.payroll_status!='1'),'00') AS salary  FROM employee e ORDER BY id DESC";
								$result = $conn->query($sql);
						}
...
<?php
		...
		if(isset($_GET['print'])){
			...
			IFNULL((SELECT loan_grant_payroll_id FROM attendance_log a WHERE a.log_date BETWEEN '$from' AND '$to' AND a.e_id=e.id AND a.status='Approved' AND a.payroll_status!='1'),'00') AS loan
